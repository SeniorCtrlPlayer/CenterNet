...
class DCN(DCNv2):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, offset_mask_padding,
                 dilation=1, deformable_groups=1, show_offset=False):
    ...
    def forward(self, input):
        ...
        if self.show_offset:
            write = SummaryWriter()
            write.add_histogram('offset_x', o1[0][0])
            write.add_histogram('offset_y', o2[0][0])
            write.add_image('offset_lt_x_image', o1[0][0].unsqueeze(dim=0))
            write.add_image('offset_lt_y_image', o2[0][0].unsqueeze(dim=0))
