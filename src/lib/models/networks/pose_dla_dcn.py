...
class DLA(nn.Module):
    ...
    def _make_conv_level(self, inplanes, planes, convs, stride=1, dilation=1):
        modules = []
        # print("test by lwk:::::{}".format(convs))
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        # state_dict = {k:v for k,v in model_weights.items() if k in self.state_dict().keys()}
        self.load_state_dict(model_weights)
        
        # self.load_state_dict({k.replace('module.',''):v for k,v in model_weights.items()})
        # self.fc = fc
